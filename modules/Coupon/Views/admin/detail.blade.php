...
@section('content')
    ...
    <style>
        ...
        body.dark-mode .btn-mint-outline:hover,
        body.dark-mode .btn-mint-outline:focus {
            border-color: #059669;
            background: #059669;
            color: #fff;
        }
        ...
        .switch-toggle input[type="checkbox"]:not(:checked)+.switch-label .switch-text-no {
            color: #888;
            display: block;
        }
        ...
        .switch-toggle input[type="checkbox"]:checked+.switch-label .switch-text-yes {
            display: block;
        }

        .switch-toggle input[type="checkbox"]:checked+.switch-label .switch-text-no {
            display: none;
        }
        ...
        body.dark-mode .custom-radio .checkmark {
            border-color: #fff;
        }
        ...
        body.dark-mode .custom-radio .checkmark:after {
        ...
        body.dark-mode .activity-item.selected {
            border-left-color: #fff;
        }
        ...
        body.dark-mode #clearSelectedService {
                    ...
                    <div class="cart-section">
                        <div class="panel">
                            <div class="panel-title"
                                style="display: flex;align-items: center;gap: 15px;border-bottom: 0;border-radius: 14px;">
                                <div style="display: flex;flex-direction: row; align-items: center;">

                                    <strong style="min-width: 60px;">{{ __('Active') }}</strong>
                                    <div class="switch-toggle" style="margin-left: 0;">
                                        <input type="checkbox" id="status_switch" name="status" value="publish"
                                            @if ($row->status == 'publish') checked @endif>
                                        <label for="status_switch" class="switch-label">
                                            <span class="switch-text-yes">YES</span>
                                            <span class="switch-text-no"
                                                style="color: #7d7d7d; font-size: 14px; font-weight: bold; margin-left: 30px; letter-spacing: 1px;">NO</span>
                                ...
                                <button class="btn btn-mint-outline" type="submit"
                                    style="margin-left: auto;display:inline-flex;align-items:center;gap:8px;font-weight:600;font-size:15px;">
                        ...
                        <div class="section-card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group" style="margin-top: 20px;">
                                    <label>{{ __('Apply Coupon To') }}<span class="text-danger">*</span></label>
                                    <div style="display: flex; gap: 15px;">
                                        <label class="custom-radio">
                                            <input type="radio" name="apply_to" value="specific_user"
                                                @if (!empty($row->apply_to) && $row->apply_to == 'specific_user') checked @endif>
                                            <span class="checkmark"></span>
                                            {{ __('Per-User') }}
                                        </label>
                                        <label class="custom-radio">
                                            <input type="radio" name="apply_to" value="first_user"
                                                @if (!empty($row->apply_to) && $row->apply_to == 'first_user') checked @endif>
                                            <span class="checkmark"></span>
                                            {{ __('First User') }}
                                        </label>
                                        <label class="custom-radio">
                                            <input type="radio" name="apply_to" value="everyone"
                                                @if (!empty($row->apply_to) && $row->apply_to == 'everyone') checked @endif>
                                            <span class="checkmark"></span>
                                            {{ __('Public') }}
                                        </label>
                                    </div>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group"
                                        ...
                                        <button id="clearSelectedService" type="button" class="btn btn-light"
                                            style="display:none;padding:6px 8px;border-radius:6px;border:1px solid #000; margin-left:10px;">{{ __('Clear') }}</button>
                                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var userSearchWrapper = $(
                '<div id="userSearchWrapper" style="position:relative;display:inline-flex;align-items:center;gap:8px;width:100%;"></div>'
            );
            var userSearchBox = $(
                '<input type="text" id="userSearch" class="form-control" placeholder="{{ __('Search by name or phone...') }}" style="display:inline-block;">'
            );
            var searchBtn = $(
                '<button type="button" id="userSearchBtn" class="btn btn-primary" style="padding:8px 20px;height:38px;display:flex;align-items:center;justify-content:center;border-radius:4px;"><i class="fa fa-search"></i></button>'
            );
            userSearchWrapper.append(userSearchBox);
            userSearchWrapper.append(searchBtn);
                ...
                $.get('{{ route('user.admin.getForSelect2') }}', {
                    ...
                    if (res && res.results && res.results.length) {
                        res.results.forEach(function(item) {
                            var img = item.avatar ? '<img src="' + item.avatar +
                                ...
                                (item.text ? item.text.trim().charAt(0).toUpperCase() : '?') +
                                '</div>';
                            html +=
